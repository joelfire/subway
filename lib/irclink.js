    ...
    Channel.all({where: {conn_id: connID}}, function (err, userChans) {
        ...
        instance.client.addListener(event, function() {
          ...
          switch (event) {
            ...
            case 'close':
              ...
              break;
           	case 'channellist':
           		// received the result of a LIST from the IRC server, from irc.js 
           		list = arguments[0];
           		names = "";
           		for (i = 0; i < list.length; i++) {
           			line = list[i].name + ', ' + list[i].users + ' users, ' + list[i].topic ;
                    instance.xmitEvent('message#', {to: 'status', from: ' ', text: line});
                    // no need to logMessage, no need for persistence of these messages
           		};
                break;
            default:
              break;
      ...
      app.ircbridge.on(connID, function(event, data) {
        if (instance.connected) {
          switch (event) {
          	case 'list':
          	  // send a LIST to IRC server, result sent back asynchronously	
